...
from .dae_epoch import shared_step as dae_shared_step
...
def run_epoch(model,
    ...
    for mode, dataloader in data_loaders.items():
        ...
        if 'train' in args.stop_based and mode == 'val':
            continue
        model, mode_metrics, total_y_mode = dae_shared_step(model, dataloader, optimizer,
                                                            criterions, args, mode, train,
                                                            epoch)
        print(f'├── {mode} finished, took {time.time() - st:.2f}s')
        if mode == 'test':
            metrics.update({
                'test_known_nodes  ': mode_metrics[0],
                'test_unknown_nodes': mode_metrics[1],
            })
        else:
            metrics.update({
    ...
    if train:
        ...
        save_checkpoint(model, optimizer, args, epoch, 'test',
                        metrics['test_unknown_nodes'][args.loss], total_y,
                        'test_unknown_nodes')
...
def save_checkpoint(model, optimizer, args, epoch, mode, now_metric, total_y,
    ...
    try:  # try-except here because checkpoint fname could be too long
        if (better_metric(now_metric, best_metric) or epoch == 0):
            ...
            print(
                f'-> New best {mode} loss at epoch {epoch}! ({metric_name} loss: {now_metric:.4f})'
            )
    except Exception as e:
        print(e)


def train_model(model, optimizer, scheduler, data_loaders, criterions, args, wandb=None):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        st = time.time()
        _, metrics, _ = run_epoch(model, data_loaders, optimizer, scheduler, criterions,
                                  args, epoch)
        ...
        for mode in metrics:
            # if 'mask' in mode and 'AE' in args.model:
            #     continue
    ...
    print(f'-> Saved best test model checkpoint at epoch {args.best_test_metric_epoch}!')
    ...
    with open(args.best_prediction_path + 'best_total_y.pkl', 'wb') as f:
        args.best_total_y['known_nodes'], args.best_total_y[
            'unknown_nodes'] = data_loaders['test'].dataset.get_know_unknow_nodes()
